...
public class HrmDetails {
	...
	public void projectDetails() throws SQLException, EncryptedDocumentException, IOException {
		...
		int res = stat.executeUpdate("insert into Project values('TY_PROJ_98086','AGG','23-8-2024','Tektest','created',5)");
		conn.close();
		...
		Sheet sh = wb.createSheet("projectdetails3");
		...
		for (int i = first; i <= last; i++) {
			try {
				...
				for (WebElement id : projectid) {
					if (id.getText().equals("TY_PROJ_98086")) {
						flag = true;
						...
						for (WebElement alldata : projectdata) {
							...
							sh = wb.getSheet("projectdetails3");
							rw = sh.getRow(0);
